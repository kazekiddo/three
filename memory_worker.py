        ...
        try:
            ...
            for char in characters:
                ...
                prompt = (
                    ...
                    "2. **重要关系事件**：识别对话中影响关系的具体事件 (closeness, trust, attraction...)。\n"
                    "3. **冲击性事件 (Shock Events)**：是否发生了重大转折？只需返回关键字：\n"
                    "   - `betrayal` (背叛/欺骗), `confession` (表白), `breakup` (分手), `reconciliation` (和解)\n"
                    "4. **关系叙事 (Narrative)**：用一句简短的话总结当前关系现状（例如：'你们的关系正在升温，但用户昨天的隐瞒让你感到一丝不安'）。\n"
                    "5. **短期情绪**：mood, anger, affection, jealousy, sadness (0-1)。\n\n"
                    f"【最近事件参考】\n{recent_events_text}\n\n"
                    f"对话记录：\n{conversation}\n\n"
                    "请以JSON格式返回：\n"
                    "{\n"
